        ...
        <div class="d-flex justify-content-between">
            ...
            <div class="text-start">
                <div class="mb-2">
                    ...
                    <span class="badge bg-success ms-2">{{ number_format($service?->grand_total) }}</span>
                </div>
                <div class="mb-2">
                    <span class="fw-bold">Paid Amount:</span>
                    <span class="badge bg-success ms-2">{{ number_format($service?->paid_amount) }}</span>
                </div>
                <div>
                    <span class="fw-bold">Due Amount:</span>
                    <span class="badge bg-info ms-2">{{ number_format($service?->due_amount) }}</span>
                </div>
                <div class="mt-2">
                    <span class="fw-bold">Paid Status:</span>
                    <span class="badge {{ $service?->paid_status == 'full_paid' ? 'bg-success' : 'bg-warning' }} ms-2">
                        {{ $service?->paid_status }}
                    </span>
                </div>
            </div>
...
<div class="mt-4">
    ...
    @if ($service->payment)
        @forelse ($service?->payment?->paymentDetails as $paymentDetail)
...
<div class="payment-form-container mt-3" style="display: none;">
    <form class="payment-form" method="POST" action="{{ route('admin.services.payment', $service->id) }}">
        @csrf
        <div class="row">
    ...
    $(document).ready(function() {
        ...
        $('.payment-form').submit(function(e) {
            ...
            if ($('.payment_type').val() === 'full_paid') {
                formData.append('amount', '{{ $service?->due_amount }}');
            }
